# Algorithms

![Algorithms](https://user-gold-cdn.xitu.io/2019/7/30/16c42528c48819fd?w=1262&h=412&f=png&s=183786)

### 简单难度
1. [二进制求和](./easy/addBinary.js)
2. [爬楼梯](./easy/climbStairs.js)
3. [报数](./easy/countAndSay.js)
4. [删除排序链表中的重复元素](./easy/deleteDuplicates.js)
5. [回文结构](./easy/isPalindrome.js)
6. [返回其最后一个单词的长度](./easy/lengthOfLastWord.js)
7. [最长公共前缀](./easy/longestCommonPrefix.js)
8. [最大和的连续子数组](./easy/maximumSubarray.js)
9. [数字是否是回文数](./easy/palindromeNumber.js)
10. [加一](./easy/plusOne.js)
11. [删除排序数组中的重复项](./easy/deleteDuplicates.js)
12. [移除元素](./easy/removeElement.js)
13. [数字反转](./easy/reverseInteger.js)
14. [罗马数字](./easy/romanToInteger.js)
15. [搜索插入位置](./easy/searchInsertPosition.js)
16. [x的平方根](./easy/sqrt.js)
17. [两数之和](./easy/twoSum.js)
18. [搜索插入位置](./easy/searchInsertPosition.js)
19. [判断括号匹配](./easy/validParentheses.js)


### 中等难度

1. [盛最多水的容器](https://github.com/bWhirring/algorithms/issues/1)
1. [整数转罗马数字](https://github.com/bWhirring/algorithms/issues/2)
2. [字符串转换整数](https://github.com/bWhirring/algorithms/issues/3)  
3. [最长回文子串](https://github.com/bWhirring/algorithms/issues/4)
4. [Z字形变换](https://github.com/bWhirring/algorithms/issues/5)



